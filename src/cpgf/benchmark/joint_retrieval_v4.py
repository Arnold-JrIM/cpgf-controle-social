...
class JointRetrievalHoldoutV4Case(BaseModel):
    ...
    def validate_contract(self) -> "JointRetrievalHoldoutV4Case":
...
class JointRetrievalHoldoutV4Suite(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    version: str = "4.0.0"
    cases: tuple[JointRetrievalHoldoutV4Case, ...]

    @model_validator(mode="after")
    def validate_suite(self) -> "JointRetrievalHoldoutV4Suite":
        if self.version != "4.0.0":
            raise ValueError("Versão do Joint Holdout 4 deve ser 4.0.0")
        if len(self.cases) != 48:
            raise ValueError("Joint Retrieval Holdout 4.0.0 deve conter exatamente 48 casos")
        ids = [case.id for case in self.cases]
        if len(ids) != len(set(ids)):
            raise ValueError("IDs duplicados no Joint Holdout 4")
...
def normalize_question_v4(text: str) -> str:
...
def load_joint_retrieval_holdout_v4(
    path: Path | str,
) -> JointRetrievalHoldoutV4Suite:
    cases: list[JointRetrievalHoldoutV4Case] = []
    with Path(path).open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in reader:
            ...
            cases.append(JointRetrievalHoldoutV4Case.model_validate(payload))
    return JointRetrievalHoldoutV4Suite(cases=tuple(cases))


def joint_holdout_v4_sha256(path: Path | str) -> str:
    return hashlib.sha256(Path(path).read_bytes()).hexdigest()


def validate_joint_holdout_v4_against_catalog(
    ...
    if non_default_gold:
        raise ValueError(
            "Documento-gabarito fora da recuperação padrão: "
            + ", ".join(non_default_gold)
        )
    return {
        "status": "PASS",
        "cases": len(suite.cases),
        "category_counts": dict(
            Counter(case.category.value for case in suite.cases)
        ),
        "expected_route_counts": dict(
            Counter(case.expected_route.value for case in suite.cases)
        ),
        "gold_documents": len(gold),
        "referenced_documents": len(referenced),
        "freshness_sensitive_cases": sum(
            case.freshness_sensitive for case in suite.cases
        ),
    }
...
def validate_joint_holdout_v4_novelty(
    suite: JointRetrievalHoldoutV4Suite,
    prior_benchmark_paths: list[Path] | tuple[Path, ...],
    *,
    max_similarity_allowed: float = 0.75,
) -> dict[str, object]:
    current = {
        case.id: normalize_question_v4(case.question) for case in suite.cases
    }
    if len(set(current.values())) != len(current):
        raise ValueError("Há perguntas duplicadas após normalização dentro do JH4")
    ...
    for path in prior_benchmark_paths:
        prior.extend(
            ...
            if (normalized := normalize_question_v4(question))
        )

    prior_set = set(prior)
    exact = sorted(
        case_id for case_id, text in current.items() if text in prior_set
    )
    if exact:
    ...
    if highest > max_similarity_allowed:
        ...
        raise ValueError(
            f"Similaridade acima do limite prospectivo "
            f"{max_similarity_allowed}: {offenders}"
        )
