...
from .joint_retrieval_v3 import JointRetrievalHoldoutV3Case, JointRetrievalHoldoutV3Suite


def _counterfactual_decision(route: Route) -> RouteDecision:
    return RouteDecision(
        route=route,
        reason="rota contrafactual para diagnóstico post-hoc do Joint Holdout 3.0",
    )


def _filter_exactness(
    case: JointRetrievalHoldoutV3Case,
    plan: object,
) -> dict[str, object]:
...
def evaluate_joint_retrieval_flow_attribution_v3(
    suite: JointRetrievalHoldoutV3Suite,
) -> dict[str, object]:
    """Decompõe post-hoc as falhas do JH3 sem alterar Router ou Planner."""
    ...
    return {
        ...
        "governance": {
            ...
            "joint_holdout_3_is_already_known": True,
