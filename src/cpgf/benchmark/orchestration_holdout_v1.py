...
ORCHESTRATION_HOLDOUT_VERSION = "1.0.0"

_CASE_ID = re.compile(r"^OH1-\d{3}$")
_CATEGORY_SOURCES = {
...
class OrchestrationHoldoutCategory(StrEnum):
    DATA_ONLY = "data_only"
...
class OrchestrationHoldoutCase(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    id: str
    category: OrchestrationHoldoutCategory
    ...
    def validate_contract(self) -> "OrchestrationHoldoutCase":
...
class OrchestrationHoldoutSuite(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    version: str = ORCHESTRATION_HOLDOUT_VERSION
    cases: tuple[OrchestrationHoldoutCase, ...]

    @model_validator(mode="after")
    def validate_suite(self) -> "OrchestrationHoldoutSuite":
        if self.version != ORCHESTRATION_HOLDOUT_VERSION:
            raise ValueError("Versão inesperada do Orchestration Holdout")
        if len(self.cases) != 56:
            raise ValueError("Orchestration Holdout 1.0.0 deve conter exatamente 56 casos")
        ids = [case.id for case in self.cases]
        if len(ids) != len(set(ids)):
            raise ValueError("IDs duplicados no Orchestration Holdout")
        expected_ids = [f"OH1-{index:03d}" for index in range(1, 57)]
        if ids != expected_ids:
            raise ValueError("IDs devem formar a sequência prospectiva OH1-001..OH1-048")
        counts = Counter(case.category.value for case in self.cases)
        required = {category.value: 8 for category in OrchestrationHoldoutCategory}
        if counts != required:
...
def load_orchestration_holdout(path: Path | str) -> OrchestrationHoldoutSuite:
    cases: list[OrchestrationHoldoutCase] = []
    with _open_csv(Path(path)) as handle:
        ...
        for row in reader:
            ...
            cases.append(OrchestrationHoldoutCase.model_validate(payload))
    return OrchestrationHoldoutSuite(cases=tuple(cases))


def orchestration_holdout_sha256(path: Path | str) -> str:
...
def _questions_from_csv(path: Path) -> list[str]:
    with _open_csv(path) as handle:
        ...
        if not reader.fieldnames or "question" not in reader.fieldnames:
            return []
        return [str(row.get("question", "")) for row in reader]


def prior_question_benchmark_paths(
    ...
    *,
    ...
    return tuple(
        ...
        if path.resolve() != current and _questions_from_csv(path)
    )


def validate_orchestration_holdout_novelty(
    ...
    *,
    ...
    if len(set(current.values())) != len(current):
        raise ValueError("Há perguntas duplicadas após normalização dentro do holdout")
...
def validate_orchestration_holdout_capabilities(
    suite: OrchestrationHoldoutSuite,
) -> dict[str, object]:
