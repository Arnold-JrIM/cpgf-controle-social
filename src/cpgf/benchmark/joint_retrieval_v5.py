...
class JointRetrievalHoldoutV5Case(BaseModel):
    ...
    def validate_contract(self) -> "JointRetrievalHoldoutV5Case":
...
class JointRetrievalHoldoutV5Suite(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    version: str = "5.0.0"
    cases: tuple[JointRetrievalHoldoutV5Case, ...]

    @model_validator(mode="after")
    def validate_suite(self) -> "JointRetrievalHoldoutV5Suite":
        if self.version != "5.0.0":
            raise ValueError("Versão do Joint Holdout 5 deve ser 5.0.0")
        if len(self.cases) != 48:
            raise ValueError("Joint Retrieval Holdout 5.0.0 deve conter exatamente 48 casos")
        ids = [case.id for case in self.cases]
        if len(ids) != len(set(ids)):
            raise ValueError("IDs duplicados no Joint Holdout 5")
...
def normalize_question_v5(text: str) -> str:
...
def load_joint_retrieval_holdout_v5(
    path: Path | str,
) -> JointRetrievalHoldoutV5Suite:
    cases: list[JointRetrievalHoldoutV5Case] = []
    with Path(path).open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in reader:
            ...
            cases.append(JointRetrievalHoldoutV5Case.model_validate(payload))
    return JointRetrievalHoldoutV5Suite(cases=tuple(cases))


def joint_holdout_v5_sha256(path: Path | str) -> str:
    return hashlib.sha256(Path(path).read_bytes()).hexdigest()


def validate_joint_holdout_v5_against_catalog(
...
def validate_joint_holdout_v5_novelty(
    suite: JointRetrievalHoldoutV5Suite,
    ...
    *,
    ...
    current = {
        case.id: normalize_question_v5(case.question) for case in suite.cases
    }
    if len(set(current.values())) != len(current):
        raise ValueError("Há perguntas duplicadas após normalização dentro do JH5")
    ...
    for path in prior_benchmark_paths:
        prior.extend(
            ...
            if (normalized := normalize_question_v5(question))
        )
