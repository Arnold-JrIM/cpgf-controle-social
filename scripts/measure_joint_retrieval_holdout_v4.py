...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Mede ou reproduz o Joint Holdout 4.0 congelado. "
            "O desempenho observado nunca determina sucesso/falha do workflow."
        )
    )
    ...
    if manifest_status not in {"FROZEN_BEFORE_MEASUREMENT", "MEASURED_INDEPENDENT"}:
        raise ValueError(f"Estado do JH4 não permite medição/reprodução: {manifest_status}")
    output_status = (
        "INDEPENDENT_MEASUREMENT"
        if manifest_status == "FROZEN_BEFORE_MEASUREMENT"
        else "INDEPENDENT_MEASUREMENT_REPRODUCTION"
    )
    if joint_holdout_v4_sha256(BENCHMARK) != manifest["benchmark"]["sha256"]:
    ...
    category_floor = criteria["per_category_joint_exact_floor"]
    criteria_evaluation = {
        "primary_joint_exact_target_met": (
            summary["joint_exact_rate"] >= criteria["primary_joint_exact_target"]
        ),
        "route_exact_floor_met": (
            summary["route_exact_rate"] >= criteria["route_exact_floor"]
        ),
        "filter_joint_exact_floor_met": (
            summary["filter_joint_exact_rate"] >= criteria["filter_joint_exact_floor"]
        ),
        "per_category_joint_exact_floor_met": all(
            item["joint_exact_rate"] >= category_floor
            for item in category_payload.values()
        ),
        "per_category_joint_exact_floor_details": {
            category: {
                "joint_exact_rate": item["joint_exact_rate"],
                "floor": category_floor,
                "met": item["joint_exact_rate"] >= category_floor,
            }
            for category, item in category_payload.items()
        },
    }
    criteria_evaluation["all_prospective_criteria_met"] = all(
        criteria_evaluation[key]
        for key in (
            "primary_joint_exact_target_met",
            "route_exact_floor_met",
            "filter_joint_exact_floor_met",
            "per_category_joint_exact_floor_met",
        )
    )

    payload = {
        "artifact": "joint_retrieval_holdout_v4_measurement",
        "version": "4.0.0",
        ...
        "governance": {
            ...
            "criteria_evaluated_after_predictions": True,
            "performance_threshold_applied_to_workflow_success": False,
