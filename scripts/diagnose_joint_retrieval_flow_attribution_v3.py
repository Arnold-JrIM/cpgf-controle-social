...
DEFAULT_HOLDOUT_MANIFEST = Path("data/manifests/joint_retrieval_holdout_3_0_0.json")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Joint Holdout 3.0 já medido. Não faz tuning nem nova alegação de generalização."
        )
    )
    ...
    if manifest["status"] != "MEASURED_INDEPENDENT":
        raise ValueError("Diagnóstico exige Joint Holdout 3.0 já medido e congelado")
    if not manifest["governance"]["holdout_is_known_after_first_measurement"]:
        raise ValueError("JH3 precisa estar explicitamente marcado como conhecido")
    if joint_holdout_v3_sha256(args.holdout) != manifest["benchmark"]["sha256"]:
        raise ValueError("Benchmark divergiu do SHA preservado no Joint Holdout 3.0")
    ...
    if _git_blob_sha(ROUTER_SOURCE) != frozen["router_source_git_blob_sha"]:
        raise ValueError("Blob do Router divergiu da medição independente do JH3")
    if _git_blob_sha(PLANNER_SOURCE) != frozen["retrieval_planner_source_git_blob_sha"]:
        raise ValueError("Blob do Planner divergiu da medição independente do JH3")

    suite = load_joint_retrieval_holdout_v3(args.holdout)
    result = evaluate_joint_retrieval_flow_attribution_v3(suite)
    ...
    if result["cases"] != summary["cases"]:
        raise AssertionError("Diagnóstico não reproduziu a cardinalidade do JH3")
    if result["actual_joint_exact"] != summary["joint_exact"]:
        raise AssertionError("Diagnóstico não reproduziu os passes independentes do JH3")
    if result["actual_joint_failures"] != len(measured["mismatch_ids"]["joint"]):
        raise AssertionError("Diagnóstico não reproduziu as falhas conjuntas do JH3")

    expected_descriptive = {
        "route_wrong_filters_exact": descriptive["route_only"],
        "route_exact_filters_wrong": descriptive["filter_only"],
        "route_wrong_filters_wrong": descriptive["route_and_filter"],
        "clean_passes": descriptive["pass"],
    }
    if result["observed_layer_mismatch_reproduction"] != expected_descriptive:
        raise AssertionError(
            "Diagnóstico não reproduziu a decomposição descritiva congelada no JH3"
        )

    first = manifest["measurement"]["first_valid_measurement"]
    payload = {
        "artifact": "joint_retrieval_flow_attribution_v3_diagnostic",
        "version": "1.0.0",
